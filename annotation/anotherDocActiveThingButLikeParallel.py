...
def getMultiScores(X_train,y_train,X_test):
	...
	#clf = MultiOutputClassifier(LogisticRegression(class_weight='balanced',random_state=0,C=21))
...
@ray.remote
def doDocument(docindex,X_annotated,y_annotated,X_undecided,posThreshold):
	outcomes = []
	
	#y_annotated = y_annotated[:,[0,1]]
	...
	#	neg_matrix[:,label_index] = neg_scores
		#print(y_with_artifical_addition[0])
		
	#print(neg_matrix[0,:])
	
	#print(y_annotated.shape)
	y_with_artifical_addition = np.vstack([y_annotated,np.zeros((1,y_annotated.shape[1]))])
	#print(y_with_artifical_addition.shape)
	neg_matrix = getMultiScores(X_annotated_plus_one,y_with_artifical_addition,X_undecided)
	#print(neg_matrix[0,:])
	#print(y_with_artifical_addition[0,:])
	...
	for label_index in range(y_annotated.shape[1]):
		...
		outcomes.append(numHasHighConfPositive)
		
	return outcomes
	
	
@ray.remote
def doDocument_slow(docindex,X_annotated,y_annotated,X_undecided,posThreshold):
	outcomes = []
	...
	for label_index in range(y_annotated.shape[1]):
		...
		scores = getMultiScores(X_annotated_plus_one, y_with_artifical_addition, X_undecided)
...
def searchForBestDocumentToAnnotate_complete(X_annotated,y_annotated,X_undecided,posThreshold):
	start = time.time()

	#outcomes = [ doDocument(docindex,X_annotated,y_annotated,X_undecided,posThreshold) for docindex in range(X_undecided.shape[0]) ]

	outcomes = [ doDocument.remote(docindex,X_annotated,y_annotated,X_undecided,posThreshold) for docindex in range(X_undecided.shape[0]) ]
...
def searchForBestDocumentToAnnotate(X_annotated,y_annotated,X_undecided,posThreshold):
	start = time.time()

	#outcomes = [ doDocument(docindex,X_annotated,y_annotated,X_undecided,posThreshold) for docindex in range(X_undecided.shape[0]) ]

	import random
	doc_indices = random.sample(range(X_undecided.shape[0]),50)

	some_outcomes = [ doDocument.remote(docindex,X_annotated,y_annotated,X_undecided,posThreshold) for docindex in doc_indices ]
...
if __name__ == '__main__':
	...
	for i in range(num_iter):
		...
		current_outcomes = searchForBestDocumentToAnnotate(current_train_X,current_y,current_unknown_X,args.posThreshold)
