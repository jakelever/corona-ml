...
if __name__ == '__main__':
	...
	parser.add_argument('--journalSearch',required=False,type=str,help='Search string for journal names')
	parser.add_argument('--titleSearch',required=False,type=str,help='Search string for title')
	parser.add_argument('--contentSearch',required=False,type=str,help='Search string for title or abstract')
	...
	if args.journalSearch:
		journalSearch = args.journalSearch.lower()
		search = [ d for d in new_documents if journalSearch in d['journal'].lower() ]
	elif args.titleSearch:
		titleSearch = args.titleSearch.lower()
		search = [ d for d in new_documents if titleSearch in d['title'].lower() ]
	elif args.contentSearch:
		contentSearch = args.contentSearch.lower()
		search = [ d for d in new_documents if contentSearch in d['title'].lower() or contentSearch in d['abstract'].lower() ]
		
	random.shuffle(search)
		
	print("Found %d documents" % len(search))
	assert len(search) > 0
